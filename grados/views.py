...
def fechas_view(request):
	form = ConsultaForm()
	return render_to_response('consulta.html', {'form': form})

@pdf
def consultafecha(request):
	form1 = ConsultaForm()
	f_inicial= form1.Fecha_Inicial
	f_cierre= form1.Fecha_Final
	#prueba= Alumno.objects.filter(fecha__range(f_inicial, f_cierre) 
	# field__gt = foo para comparar > ; # field__lt = foo para comparar <;
	#prueba= Alumno.objects.filter(fecha__lt = datetime.datetime.today() )
	#prueba= Alumno.objects.filter(fecha__lt = datetime.datetime('01/01/2008') )
	prueba = Alumno.objects.filter(fecha__range(f_inicial,f_cierre))
